...
bool DeviceConfig::loadFromJson(const std::string& jsonStr) {
    try {
        ...
        // Parse AI reasoning settings
        enableAIReasoning = j.value("enableAIReasoning", enableAIReasoning);
        reasoningConfidenceThreshold = j.value("reasoningConfidenceThreshold", reasoningConfidenceThreshold);
        reasoningInterval = j.value("reasoningInterval", reasoningInterval);
        enableCrossCameraAnalysis = j.value("enableCrossCameraAnalysis", enableCrossCameraAnalysis);
...
std::string DeviceConfig::toJson() const {
    ...
    j["enableAIReasoning"] = enableAIReasoning;
    j["reasoningConfidenceThreshold"] = reasoningConfidenceThreshold;
    j["reasoningInterval"] = reasoningInterval;
    j["enableCrossCameraAnalysis"] = enableCrossCameraAnalysis;
...
bool GlobalConfig::loadFromJson(const std::string& jsonStr) {
    try {
        ...
        // Parse LLM settings
        enableLLMIntegration = j.value("enableLLMIntegration", enableLLMIntegration);
        llmApiKey = j.value("llmApiKey", llmApiKey);
        llmModelName = j.value("llmModelName", llmModelName);
        llmApiEndpoint = j.value("llmApiEndpoint", llmApiEndpoint);
        llmMaxTokens = j.value("llmMaxTokens", llmMaxTokens);
        llmTemperature = j.value("llmTemperature", llmTemperature);
        llmRequestTimeoutSecs = j.value("llmRequestTimeoutSecs", llmRequestTimeoutSecs);
...
bool GlobalConfig::saveConfig() {
    try {
        ...
        // LLM settings
        j["enableLLMIntegration"] = enableLLMIntegration;
        j["llmApiKey"] = llmApiKey;
        j["llmModelName"] = llmModelName;
        j["llmApiEndpoint"] = llmApiEndpoint;
        j["llmMaxTokens"] = llmMaxTokens;
        j["llmTemperature"] = llmTemperature;
        j["llmRequestTimeoutSecs"] = llmRequestTimeoutSecs;
...
void parseSettings(const nx::sdk::ISettingsResponse* settings, std::shared_ptr<DeviceConfig> config) {
    ...
    // Parse AI reasoning settings
    bool enableAI = settings->getBool("enableAIReasoning", config->enableAIReasoning);
    config->enableAIReasoning = enableAI;

    float reasoningThreshold = settings->getFloat("reasoningConfidenceThreshold",
                                                 config->reasoningConfidenceThreshold);
    config->reasoningConfidenceThreshold = reasoningThreshold;

    int reasoningInterval = settings->getInt("reasoningInterval", config->reasoningInterval);
    config->reasoningInterval = reasoningInterval;

    bool crossCamera = settings->getBool("enableCrossCameraAnalysis", config->enableCrossCameraAnalysis);
    config->enableCrossCameraAnalysis = crossCamera;
